...
//	}
	except_A.x  = MAX_CTRL_ANGLE  *( my_deathzoom( ( RX_CH[ROL]) ,30 )/500.0f );   //30
	except_A.y  = MAX_CTRL_ANGLE  *( my_deathzoom( (-RX_CH[PIT]) ,30 )/500.0f );  //30
	if( Thr_Low == 0 )
	{
		except_A.z += (s16)( MAX_CTRL_YAW_SPEED *( my_deathzoom_2( (RX_CH[YAW]) ,40 )/500.0f ) ) *T ;  //50
	}	
	else
	{
...
//	posture_value[0] =    out_pitch ;
	posture_value[1] =  0;//- out_yaw ;
	posture_value[2] =  0;//  out_yaw ;
	d3=out_yaw;
	d4=Yaw;
...
//	curve[3] = (0.55f + 0.45f *ABS(posture_value[3])/1000.0f) *posture_value[3] ;
	...
	curve[2] = (0.55f + 0.45f *ABS(posture_value[2])/1000.0f) *posture_value[2] ;
//CH_filter
//  motor[0] = my_deathzoom( ( RX_CH[THR]) ,50 ) + Thr_Weight *curve[0] ;
//	motor[1] = my_deathzoom( ( RX_CH[PIT]) ,50 ) + Thr_Weight *curve[1] ;
//	motor[2] = my_deathzoom( ( RX_CH[PIT]) ,50 ) + Thr_Weight *curve[2] ;
  motor[0] = RX_CH[THR] + Thr_Weight *curve[0] ;
	motor[1] = RX_CH[PIT] + Thr_Weight *curve[1] ;
	motor[2] = RX_CH[PIT] + Thr_Weight *curve[2] ;
...
//		}
	...
	motor_out[0] = (s16)(motor[0])+450;
	motor_out[1] = (s16)(motor[1])+450;
	motor_out[2] = (s16)(motor[2])+450;
